 ...
 */
 ...
 export class DatasService {
   ...
   /**
    * TODO:
    * 1.Remove all actions and return responses as observable.
    * 2.Put all action in datasEffects
    *   to keep all state logic into the state module.
    */
   ...
   getDatasArray(_params):Observable<any> {
     return Observable.create((observer) => {
       ...
       if(!storage){
         observer.next({ type: DatasActions.GET_DATAS_ARRAY_FAILED })
       }
       ...
       this.http.get(`${this.apiEndPoint}${_params.path}`, options)
                ...
                .subscribe(
                   datas => {
                     observer.next({ type: DatasActions.GET_DATAS_ARRAY_SUCCESS, payload: datas })
                   },
                   (error) => {
                       console.log(' ERROR: ' + error);
                       observer.next({ type: DatasActions.GET_DATAS_ARRAY_FAILED, payload: error })
                   });
     })
   }
   ...
   update(_query:any):Observable<any> {
     ...
     return Observable.create((observer) => {
       this.http.put(url, body, options)
                ...
                .subscribe(
                    data => {
                       observer.next({ type: DatasActions.UPDATE_DATA_SUCCESS, payload: {response: data.response, queryParams:_query} })
                    },
                    (error:any) => {
                      // format data error
                      let msg:string = `${(error.statusText)? error.statusText + ' Could not update item' : 'Could not update item'}`
                      observer.next({ type: DatasActions.UPDATE_DATA_FAILED, payload: msg })
                    }
                ); // Eof subscribe
     });
   }
   ...
   delete(_query):Observable<any> {
     ...
     return Observable.create((observer) => {
       this.http.delete(url, headers)
                .subscribe(
                  (data:any) => {
                     observer.next({ type: DatasActions.DELETE_DATA_SUCCESS, payload: {response: data, queryParams:_query} })
                   },
                   (error) => {
                     let msg:string = `${(error.statusText)? error.statusText + ' Could not delete item' : 'Could not delete item'}`
                     observer.next({ type: DatasActions.DELETE_DATA_FAILED, payload: msg })
                   }
                );
     });
   }
   ...
   create(_query: any):Observable<any>  {
     return Observable.create((observer) => {
       ...
       let headers:Headers = new Headers({'cache-control': 'no-cache','x-access-token': storage, 'Content-Type': 'application/json'});
       let options:RequestOptions = new RequestOptions({ headers: headers });
       ...
       this.http.post(`${this.apiEndPoint}${_query.path}`, body, options)
                ...
                .subscribe(
                   data => {
                     ...
                     observer.next({ type: DatasActions.CREATE_DATA_SUCCESS, payload: data })
                   },
                   (error) => {
                     let msg:string = `${(error.statusText)? error.statusText + ' Could not create item' : 'Could not create item'}`
                     observer.next({ type: DatasActions.CREATE_DATA_FAILED, payload: msg })
                   }
                );
     })
   }
