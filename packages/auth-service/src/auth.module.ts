...
import { authenticationMiddleware } from './auth.authorization.middleware';
...
export class AuthModule {
  ...
  private async _initServer(): Promise<AuthServer> {
    // init server
    if (!this.serverConfig) return Promise.reject({code: 500, message: 'Server need serverConfig'});
    return this.server = new AuthServer({
      serverConfig: this.serverConfig,
      serviceName: this.serviceName
    });
  }

  private async _initRepo(): Promise<AuthRepo> {
    // init repository
    if (!this.db || !this.serverConfig) return Promise.reject({code: 500, message: 'Repository need DB'});
    return this.repository = await new AuthRepo().init(
      this.db,
      this.serverConfig
    );
  }

  private async _initRouter(): Promise<AuthRouter> {
    // init router
